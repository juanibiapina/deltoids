...
- **Draw budget defers heavy rendering during navigation.** Each frame the shell (`cli/browse/mod.rs`) derives a `DrawBudget` (`mode.rs`) from whether the last input burst was empty: `Fast` while a navigation key is held (non-empty burst), `Full` once input settles (empty burst = poll timeout); while streaming the poll shrinks to `SETTLE_TIMEOUT` so the settled `Full` frame lands promptly after release. Both modes honor it through the shared `should_build_body` predicate (`mode.rs`): on a `Fast` frame skip syntax-highlighting a not-yet-cached unit and show a cheap header placeholder; on the settling `Full` frame build and retain it. The deferred unit differs per mode. **Traces** (`traces/detail.rs`) keys its `DiffCache` by `(trace_index, entry_index)` and defers one entry's diff (`render_detail_placeholder`). **Files** (`files/diff_pane.rs`) keys its `DiffCache` by input file index and defers per selected file: `assemble_window` walks the sidebar's selection range (one file, or a directory subtree) and, per file, uses the retained highlighted block, builds+retains it (`Full`), or emits a fixed-height placeholder block (`placeholder_file_block`, `Fast` miss). Because holding `j` in the sidebar surfaces one new file per step, the win is automatic. Both caches clear on width change or reload; revisiting a rendered unit is a cache hit, not a re-highlight. Scroll math is window-relative: `assemble_window` records `window_rows`, and scroll clamps to it (a directory subtree's scrollbar refines as placeholder heights resolve to real ones on `Full`, matching Traces).
...
- **Scroll feel is one file.** All wheel-burst smoothing lives in `scroll.rs` (`WheelScroll`); both TUIs route wheel events through it, so changing scroll feel is a one-file edit they both inherit. The wheel scrolls whichever pane the cursor hovers, but holding Ctrl redirects the scroll to the mode's list pane regardless of hover (Files → sidebar file list, Traces → entries/edits list); that redirect lives in each mode's `handle_mouse`, not `scroll.rs`. Ctrl is used rather than Shift because common terminals/tmux swallow Shift+wheel as a mouse-mode bypass and never forward it to the app.
