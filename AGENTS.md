...
- **Structural views**: `deltoids` CLI gets `-s/--summary`, `-S/--summary-then-diff`, `-p/--public`, `--signatures-only`. `rv` cycles `Full → Outline → Summary` with `v` and toggles public-only with `p`. The Outline view is a document-symbols panel (kind icons + tree branches + italic signature) with row backgrounds tinted by each symbol's diff status (computed by `structural::outline`). The Summary view shows the human change descriptions. `edit-tui` toggles a per-entry summary with `s` (uses each hunk's deepest breadcrumb ancestor since the trace doesn't carry full source).
