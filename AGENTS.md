...
crates/
  ...
  deltoids-cli/
    ...
    src/cli/browse/          # unified scrolling TUI (files / traces / live)
      ...
      traces/                #   TracesMode (edit/write trace browser)
        ...
        test_support.rs      #     shared test fixtures
      live/                  #   LiveMode (ephemeral working-tree edit feed)
        mod.rs               #     LiveMode impl of Mode
        model.rs             #     LiveFeed engine + ingest
        feed_pane.rs         #     feed list slice
        detail.rs            #     detail/diff slice (cache + render)
        test_support.rs      #     shared test fixtures
...
- **One unified TUI, three modes.** `deltoids tui` (and bare `deltoids` on a TTY) opens the TUI (`cli/browse/`); the starting mode is smart (`browse::smart_initial_mode`): **Files** mode (`files/`) when the working tree has local changes, otherwise **Traces** mode (`traces/`). **Live** mode (`live/`) is reached by cycling. `]` cycles the left panel to the next mode and `[` to the previous, wrapping (Files → Traces → Live → Files), lazygit-style; the shell indexes modes by `[_; MODE_COUNT]` arrays (`MODE_COUNT == TAB_LABELS.len()`). (When stdout is not a TTY, `tui` falls back to the Traces headless scripted render, the `tui_cli.rs` contract; Live is interactive-only.) The active mode's top-left panel title is a lazygit-style tab strip `─[1]─Files - Traces - Live─` (`render_tui::pane_block_with_tabs`), with the active label bold-accent and the inactive ones muted. The `Mode` seam (`mode.rs`) is a real seam: the shell (`mod.rs`) is mode-agnostic and owns the terminal, the event loop, the one draggable divider, the shared sidebar width, `<`/`>` resize, the help popup, the `[`/`]` cycling, and reload orchestration; each mode owns its full vertical slice (state, keys, mouse, render, reload) behind the trait. Pane chrome still comes from `render_tui`; modes scroll by physical rows and hard-wrap long diff lines onto padded continuation rows. Live reload keeps the active mode fresh eagerly (after a debounce) and reloads the inactive modes lazily on their next activation. Every mode builds lazily, including the one shown at startup: the shell starts with empty placeholders, flips the active tab, draws a `Loading…` frame (`draw_loading`, the tab strip plus a centered message), then builds the mode on the next loop step. Neither launch nor a cycle blocks on a blank screen, and an inactive mode never pays its build/watcher cost (Files/Live working-tree diff and recursive watcher, which scale with repo size) until first activated. The build is synchronous (the data crosses no thread), so input pauses during it, but the visible `Loading…` state shows why. Sidebar width is shell-level and shared across modes.
- **Live is a complement to Traces, not a replacement.** **Traces** capture agent *intent* (per-tool-call boundaries, a `reason` per edit, tool attribution, failed edits) and persist under `$XDG_DATA_HOME/edit/traces/`, fed by the pi extension and Claude Code hook. **Live** observes the filesystem directly and needs no plugin or agent integration — the "works out of the box for any tool that writes files" path. Its engine (`live/model.rs`, `LiveFeed::ingest`) reuses `build_model`, so git supplies each first-seen file's `HEAD` baseline for free (a dirty repo emits one initial `HEAD`→working entry per file, a clean repo emits nothing) and an in-memory `latest_state` cache supplies incremental deltas thereafter (each later entry diffs the last-known state, not cumulative-vs-`HEAD`). Startup and every later change run the same `ingest`; there is no full-repo scan and memory holds only files changed this session. Live is ephemeral: no persistence, no id, no grouping unit. Do **not** wire Live into the trace store or drop the plugins — the two modes answer different questions and keep separate data.
- **Shared workdir watcher is one file.** Files and Live both watch the repo working tree and filter the same noise (`.git/` churn, gitignored paths). That essence lives once in `browse/watch.rs` (`spawn_workdir_watcher`, `path_warrants_reload`, `is_git_internal`); both modes delegate rather than duplicate. Files mode's `DiffSource::WorkingTree` arm and Live mode both call it; Files' `Static` arm keeps returning "no watcher / never reload".
