...
- **Structural views**: `deltoids` CLI gets `-s/--summary`, `-S/--summary-then-diff`, `-p/--public`, `--signatures-only`. `rv` cycles `Full → Outline → Summary` with `v` and toggles public-only with `p`. The Outline view renders real new-side source code with function bodies elided to `{ … }`, syntax-highlighted via `deltoids::render_tui::highlighted_spans`, with each line's background painted by the diff status of its enclosing symbol (computed by `structural::outline`). The Summary view shows the human change descriptions. `edit-tui` toggles a per-entry summary with `s` (uses each hunk's deepest breadcrumb ancestor since the trace doesn't carry full source).
