...
- **Draw budget defers heavy rendering during navigation.** Each frame the shell (`cli/browse/mod.rs`) derives a `DrawBudget` (`mode.rs`) from whether the last input burst was empty: `Fast` while a navigation key is held (non-empty burst), `Full` once input settles (empty burst = poll timeout); while streaming the poll shrinks to `SETTLE_TIMEOUT` so the settled `Full` frame lands promptly after release. Traces mode (`traces/detail.rs`) honors it via `should_build_body`: on a `Fast` frame it skips syntax-highlighting a not-yet-cached entry and shows a cheap header placeholder (`render_detail_placeholder`), so holding `j` over large edits never blocks on the per-line syntect cost. Rendered entries are **retained** in a keyed `DiffCache` (keyed by `(trace_index, entry_index)`, cleared on width change or reload), mirroring Files mode's up-front `build_view` cache, so revisiting an entry is instant index-swapping instead of a re-highlight. Files mode's draw is already cheap (its diff is built on reload, not draw), so it ignores the budget.
